...
def get_model_from_run(run_path, step=-1, only_conf=False, device="cpu"):
    ...
    model = models.build_model(conf.model).to(device)
...
def eval_batch(model, task_sampler, xs, xs_p=None, device="cpu"):
    task = task_sampler()
...
def build_evals(conf):
    ...
    for strategy in ["random_quadrants", "orthogonal_train_test", "overlapping_train_test"]:
        evaluation_kwargs[strategy] = {"prompting_strategy": strategy}

    for method in ["half_subspace", "skewed"]:
        eigenvals = (
            torch.zeros(n_dims) if "subspace" in method else 1 / (torch.arange(n_dims) + 1)
        )
        if "subspace" in method:
            eigenvals[: n_dims // 2] = 1
        scale = sample_transformation(eigenvals, normalize=True)
        evaluation_kwargs[f"{method}"] = {"data_sampler_kwargs": {"scale": scale}}
...
def compute_evals(all_models, evaluation_kwargs, save_path=None, recompute=False, device="cpu"):
    try:
...
def get_run_metrics(
    ...
    return compute_evals(all_models, evaluation_kwargs, save_path, recompute, device=device)


# Everything below unchanged
def conf_to_model_name(conf):
    if conf.model.family == "gpt2":
        return {(3, 2): "Transformer-xs", (6, 4): "Transformer-small", (12, 8): "Transformer"}[
    ...
    for task in os.listdir(run_dir):
        ...
        for run_id in os.listdir(task_dir):
            ...
            params = {
                "run_id": run_id,
                "task": task,
                "model": conf_to_model_name(conf),
                "kwargs": "_".join(f"{k}={v}" for k, v in conf.training.task_kwargs.items()),
                "num_tasks": conf.training.get("num_tasks", -1),
                "num_examples": conf.training.get("num_training_examples", -1),
                "n_dims": conf.model.n_dims,
...
if __name__ == "__main__":
    ...
    for task in os.listdir(run_dir):
        ...
        for run_id in tqdm(os.listdir(task_dir)):
            ...
            metrics = get_run_metrics(run_path, device=device)
